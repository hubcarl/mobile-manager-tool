...
public class QImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > GalleryActivity.screenWidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
		} else if(height > GalleryActivity.screenHeight){
			center(true, false);
		}else{
			center(true, true);
		}
	...
	private void arithScaleRate() {
		float scaleWidth = GalleryActivity.screenWidth / (float) imageWidth;
		float scaleHeight = GalleryActivity.screenHeight / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,GalleryActivity.screenWidth / 2f, GalleryActivity.screenHeight / 2f);
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = GalleryActivity.screenWidth - width;
		float fill_height = GalleryActivity.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( GalleryActivity.screenWidth/2f)/imageWidth;
