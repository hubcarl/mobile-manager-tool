...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
		...
		if (TextUtils.isEmpty(uri)) {
			...
			if (options.shouldShowImageForEmptyUri()) {
                Bitmap imageBitmapOnFail = options.getImageBitmapOnFail(configuration.resources);
                if (options.getDisplayer() instanceof RoundedBitmapDisplayer && imageBitmapOnFail != null) {
                    int corner = ((RoundedBitmapDisplayer)options.getDisplayer()).getRoundedBitmapCorner();
                    imageAware.setImageDrawable(new RoundedBitmapDisplayer.RoundedDrawable(imageBitmapOnFail,corner, 0));
                } else {
		...
		if (bmp != null && !bmp.isRecycled()) {
			...
			if (options.shouldPostProcess()) {
				com.nostra13.universalimageloader.core.ImageLoadingInfo imageLoadingInfo = new com.nostra13.universalimageloader.core.ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
			...
			if (options.shouldShowImageOnLoading()) {
                Bitmap bitmapOnLoading = options.getImageBitmapOnLoading(configuration.resources);
                if (options.getDisplayer() instanceof RoundedBitmapDisplayer && bitmapOnLoading != null) {
                    int corner = ((RoundedBitmapDisplayer)options.getDisplayer()).getRoundedBitmapCorner();
                    imageAware.setImageDrawable(new RoundedBitmapDisplayer.RoundedDrawable(bitmapOnLoading,corner, 0));
                } else {
			...
			com.nostra13.universalimageloader.core.ImageLoadingInfo imageLoadingInfo = new com.nostra13.universalimageloader.core.ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
					options, listener, progressListener, engine.getLockForUri(uri));
			com.nostra13.universalimageloader.core.LoadAndDisplayImageTask displayTask = new com.nostra13.universalimageloader.core.LoadAndDisplayImageTask(engine, imageLoadingInfo,
					defineHandler(options));
			if (options.isSyncLoading()) {
				displayTask.run();
			} else {
    ...
    /**
     * get ImageLoader max ImageSize
     * @return ImageSize
     */
    public ImageSize getMaxImageSize() {
        return configuration.getMaxImageSize();
    }
}
